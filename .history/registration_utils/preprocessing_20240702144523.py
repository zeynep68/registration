...
class DataPreparation:
	...
	def rescale_img(moving, scaling_factor, interpolation=None):
		if interpolation is None:
			interpolation = cv2.INTER_CUBIC

		return  cv2.resize(moving, (0,0), fx=scaling_factor, 
                     	   fy=scaling_factor, interpolation=interpolation)
	...
	def preprocess(self, data, normalize_value=1.0, use_trans_mask=False, return_torch_tensor=True):
		...
		moving = rescale_img(moving, self.scaling_factor, fixed.shape)
